<script>
	...
	let y;
	let colours = sections.reverse().map(x => x.colour);

	let sectionBackground = () => sections.forEach(_, index => Viewport.Height * (index - 1) <= y < Viewport.Height * index ? colours[index] : '#fff');
</script>
...
>
	...
	<div
		...
		class="min-w-full min-h-screen z-0"
