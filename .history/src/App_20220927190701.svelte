<script>
	...
	const colours = sections.filter(section => section.colour);
	...
	let setColour = () => {
		...
		for (let index = 0; index < sections.length; index++) {
			(outerHeight * index) <= y < (outerHeight * (index + 1))
				...
				: 'blueviolet';
		console.log((outerHeight * index), (outerHeight * (index + 1)));
		}
		console.log(outerHeight, y, colour)
		backgroundColour = colour;
	};
</script>

<main
	class="min-w-full min-h-screen flex flex-col scrollsnap-container scroll-smooth z-10"
	on:scroll={setColour}
>
	<div
		class="page-bg min-w-full min-h-screen z-0 absolute"
		style="--background: {backgroundColour}"
	/>

	<div
		class="min-w-full p-6 flex flex-row justify-between items-center z-100 fixed"
	>
		<div>
	...
	<div
		class="min-w-full p-6 flex flex-row justify-between items-end z-100 fixed bottom-0 left-0 pointer-events-none"
	>
