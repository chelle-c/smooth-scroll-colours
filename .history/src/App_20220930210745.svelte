<script>
	...
	let setColour = () => {
		...
		bgColour = colour;
		document.getElementById('background').style.backgroundColor = bgColour;
		lastScroll = yTop;
		isScrolling = false;
	};
...
>
	...
	{#each [...sections].reverse() as { id, image, alt }}
		<section
			id="section{id}"
			class="flex flex-col justify-center items-center min-w-full min-h-screen justify-center items-center transition-colors snap-normal snap-center"
		>
